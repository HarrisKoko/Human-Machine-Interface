...
def calibrate(intlet):
    ...
    x_drift = determine_drift(timestamps, datax)
    y_drift = determine_drift(timestamps, datay)
    z_drift = determine_drift(timestamps, dataz)
...
def main():
    ...
    while run:
        ...
        sample[1] = sample[1] - calibration[1][0]
        sample[2] = sample[2] - calibration[2][0]
        ...
        #if(len(datax) > 1):
            #sumx.append(1/2 * (datax[len(datax) - 2] + datax[len(datax) - 1]) * (timestamp - old_time))
            #sumy.append(1/2 * (datay[len(datay) - 2] + datay[len(datay) - 1]) * (timestamp - old_time))
            #sumz.append(1/2 * (dataz[len(dataz) - 2] + dataz[len(dataz) - 1]) * (timestamp - old_time))
        old_time = timestamp
