...
class GripStrengthPredictor:
    ...
    def forwardPropogation(self):
        with torch.no_grad(): # we don't need gradients in the testing phase
            if torch.cuda.is_available():
                self.predicted = self.model(Variable(torch.from_numpy(self.x_test).cuda())).cpu().data.numpy()
            else:
                self.predicted = self.model(Variable(torch.from_numpy(self.x_test))).data.numpy()
